(function() {
	...
	var url = 'undefinedSpellCheckUrl';
	var maxRequest = 200;
			...
			var wiggles_filter = {
				elements: {
					span: function(el) {
						...
						var isSpellcheck = !!el.attributes && (el.attributes['class'] == "spellcheck-typo" || el.attributes['class'] == "spellcheck-typo-disabled");
						if (isSpellcheck) {
...
//function(){this.instanceReadyHelper(editor, this);
			...
			if (editor && !editor.config.spellchecker) {
				editor.config.spellchecker = {};
			}
			*/
			...
			/*
			settings = editor.config.spellchecker;
			if (!settings) {
				settings = {};
			}
			*/

			lang = editor.config.spellCheck_language || lang;
			url = editor.config.spellCheck_url || url;
...
//			});
			...
			var generateSuggestionMenuItem = function(suggestion, typo, element) {
				return {
					...
					onClick: function() {
						...
						element.$.className = "spellcheck-typo-disabled";
					}
				};
			};
			...
			editor.addMenuGroup('spellcheck', -10 * 3); /*at the top*/
			editor.addMenuGroup('spellchecktools', -10 * 3 + 1);

			if(!editor.contextMenu.hasSpellcheckListener){

                editor.contextMenu.addListener(function(element) {
                    if (!element.$ || !element.$.className || element.$.nodeName.toLowerCase() != 'span' || element.$.className !== "spellcheck-typo") {
                        return;
                    }
                    ...
                    if (suggestions.length == 0) {
                        ...
                        for (var i = 0; i < suggestions.length; i++) {
                            ...
                            editor.addMenuItem('spellcheck_sug_' + i, generateSuggestionMenuItem(word, !!!i, typo, element));
                            retobj["spellcheck_sug_" + i] = CKEDITOR.TRISTATE_OFF;
                        }
                    }

                    editor.addMenuItem('spellcheck_ignore', {
                        label: editor.lang.spellchecker.ignore,
                        group: 'spellchecktools',
                        onClick: function() {
                            ignoreWord(element.$, typo, true);
                        }
                    });

                    retobj["spellcheck_ignore"] = CKEDITOR.TRISTATE_OFF;
                        //
                    /*
                    if (localStorage ) {
                ...
                editor.contextMenu.hasSpellcheckListener = true;
			}
		}
		/* #2 setup layer */
	...
	var start = function() {
		...
		editor.getCommand('spellcheck').setState(CKEDITOR.TRISTATE_ON);
		state = true;
	...
	var stop = function() {
		editor.getCommand('spellcheck').setState(CKEDITOR.TRISTATE_OFF);
		state = false;
		clearAllSpellCheckingSpans(editor.getBody());
	};

		editor.stopSpellchecker = function(){
		    stop();
		};
		editor.startSpellchecker = function(){
		    start();
		};
	...
	function keyHandler(ch8r) {
		...
		if (target.$.className == "spellcheck-typo") {
			target.$.className = 'spellcheck-typo-disabled';
		}
	...
	function send(words) {
		...
		sendSpellcheckData(data, callback);
	}

	function wordsToRPC(words, lang) {
		return '{"locale":"' + lang + '","words":["' + words.join('","') + '"]}';
	}

	function sendSpellcheckData(data, callback) {
		$.ajax({
			url: url,
			method: 'POST',
			dataType: 'json',
	...
	*/
	...
	function parseRpc(result, words) {
	/*
	...
	function clearAllSpellCheckingSpans(base) {
		...
		while (!finished) {
			...
			while (i--) {
				node = nodes[i];
				if (node.className == ('spellcheck-typo') || node.className == ('spellcheck-typo-disabled')) {
					unwrapbogus(node);
					finished = false;
				}
			}
	...
	function appendCustomStyles(path) {
		if (!editor.getDoc().getElementById('spellchecker_theme')) {
			var head = editor.getDoc().getElementsByTagName("head")[0];
			var element = editor.getDoc().createElement("link");
			element.setAttribute("rel", "stylesheet");
			element.setAttribute("type", "text/css");
			element.setAttribute("href", path + "/theme/spellchecker.css");
			element.setAttribute("id", 'spellchecker_theme');
	...
	*/
	...
	function getWords(corpus, max) {
		...
		for (var i = 0; i < allTextNodes.length; i++) {
			...
			if (allTextNodes[i].parentNode && allTextNodes[i].parentNode.className && allTextNodes[i].parentNode.className == ("spellcheck-typo")) {
				fullTextContext += "";
			} else {
				fullTextContext += " ";
			}
	...
	function validWordToken(word) {
		...
		var ingnoreAllCaps = true; //(settings.ignore_block_caps === true);
		var ignoreNumeric = true; //(settings.ignore_non_alpha !== false);
	...
	*/
	...
	function MarkTypos(textNode) {
		...
		while ((match = regex.exec(currentNode.data)) != null) {
			...
			span.className = "spellcheck-typo";
	...
	function ignoreWord(target, word, all) {
		if (all) {
			...
			var allInstances = editor.document.find('span.spellcheck-typo').$;
